...
export default function AssetsPage() {
  ...
  const { assets, createItem, updateItem, deleteItem, createSystemAlert } =
    useUserFinanceData();
  ...
  async function addAsset(event) {
    ...
    try {
      ...
      setSuccess({
        ...
        description: "El activo se guardó en Firestore.",
      });
    } catch (err) {
  ...
  async function handleEdit(values) {
    ...
    try {
      ...
      await updateItem("assets", editing.id, {
        name: nextName,
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      await deleteItem("assets", deleting.id);
      setSuccess({
        title: "Activo eliminado",
        description: "El activo se eliminó.",
      });
    } catch (err) {
      setError(err.message || "No se pudo eliminar el activo.");
    }
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Registra bienes e inversiones que forman parte de tu patrimonio.
        </p>
      </div>
      {error ? (
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader>
            <CardDescription>Patrimonio registrado</CardDescription>
            <CardTitle className="text-2xl text-primary">
              {formatCurrency(total)}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-muted-foreground">
            Suma del valor actual de tus activos.
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardDescription>Promedio por activo</CardDescription>
            <CardTitle className="text-2xl text-primary">
              {formatCurrency(assets.length ? total / assets.length : 0)}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-muted-foreground">
            Valor promedio registrado.
          </CardContent>
        </Card>
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="min-w-0 lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Valor actual de bienes e inversiones.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {assets.length ? (
              ...
              <p className="py-10 text-center text-sm text-muted-foreground">
                No hay activos todavía. Agrega bienes o inversiones desde el
                formulario.
              </p>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Guarda un bien o inversión.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        fields={[
          ...
          },
          {
            ...
            options: categories.map((item) => ({
              value: item,
              label: ASSET_CATEGORY_LABELS[item],
            })),
          },
          {
            id: "current",
