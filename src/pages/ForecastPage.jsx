...
export default function ForecastPage() {
  ...
  const projection = [
    ...
    ...forecast.map((item) => ({
      month: item.month,
      incomeForecast: item.incomeForecast,
      expenseForecast: item.expenseForecast,
    })),
  ];
  ...
  const rows = forecast.map((item, index) => {
    const previous =
      index === 0
        ? {
            incomeForecast: history.at(-1)?.income || 0,
            expenseForecast: history.at(-1)?.expenses || 0,
            savings:
              (history.at(-1)?.income || 0) - (history.at(-1)?.expenses || 0),
          }
        : forecast[index - 1];
    return {
      ...item,
      incomeChange: changePercent(item.incomeForecast, previous.incomeForecast),
      expenseChange: changePercent(
        item.expenseForecast,
        previous.expenseForecast,
      ),
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Una estimación determinística de tus ingresos, gastos y ahorro para
          los próximos cuatro meses.
        </p>
      </div>
      {!hasHistory ? (
        <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
          Agrega ingresos y gastos para generar un pronóstico útil. Mientras
          tanto, los valores permanecen en cero.
        </div>
      ) : null}
      <div className="grid gap-4 md:grid-cols-3">
        <FinanceMetricCard
          ...
          value={formatCurrency(last.incomeForecast)}
          detail="Basado en ingresos recientes."
        />
        <FinanceMetricCard
          icon={TrendingDown}
          label="Gastos estimados"
          value={formatCurrency(last.expenseForecast)}
          detail="Basado en gastos recientes."
          tone="amber"
        />
        <FinanceMetricCard
          icon={PiggyBank}
          label="Ahorro estimado"
          value={formatCurrency(last.savings)}
          detail="Ingresos menos gastos proyectados."
          tone="blue"
        />
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Perspectiva de tu dinero</CardTitle>
          <CardDescription>
            Líneas sólidas para meses registrados y punteadas para la
            proyección.
          </CardDescription>
        </CardHeader>
        <CardContent className="h-80 min-w-0">
          {hasHistory ? (
            ...
            <div className="flex h-full items-center justify-center rounded-lg border border-dashed text-center text-sm text-muted-foreground">
              Agrega ingresos y gastos para ver la gráfica de pronóstico.
            </div>
          )}
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cuánto podrían cambiar tus ingresos, gastos y ahorro contra el mes
            anterior.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          <div className="hidden grid-cols-7 gap-4 px-4 pb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground lg:grid">
            <span>Mes</span>
            <span>Ingresos</span>
            <span>Cambio</span>
            <span>Gastos</span>
          ...
          {rows.map((row) => (
            <div
              key={row.month}
              className="grid gap-3 rounded-lg border-l-4 border-l-primary bg-muted/20 p-4 lg:grid-cols-7 lg:items-center"
            >
              ...
              <div>
                ...
                <p className="font-medium tabular-nums text-primary">
                  {formatCurrency(row.incomeForecast)}
                </p>
              </div>
              <div>
                <span className="text-xs text-muted-foreground lg:hidden">
                ...
                <ChangeValue value={row.incomeChange} />
              </div>
              <div>
                <span className="text-xs text-muted-foreground lg:hidden">
                  Gastos
                ...
                <p className="font-medium tabular-nums text-amber-600 dark:text-amber-400">
                  {formatCurrency(row.expenseForecast)}
                </p>
              </div>
              <div>
                <span className="text-xs text-muted-foreground lg:hidden">
                ...
                <ChangeValue value={row.expenseChange} reverse />
              </div>
              <div>
                <span className="text-xs text-muted-foreground lg:hidden">
                  Ahorro
              ...
              <div>
                ...
                <ChangeValue value={row.savingsChange} />
              </div>
            </div>
          ))}
        </CardContent>
