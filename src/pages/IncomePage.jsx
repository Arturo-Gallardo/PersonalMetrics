...
export default function IncomePage() {
  ...
  const { income, createItem, updateItem, deleteItem, createSystemAlert } =
    useUserFinanceData();
  ...
  async function addIncome(event) {
    ...
    try {
      ...
      setSuccess({
        ...
        description: "El ingreso se guardó en Firestore.",
      });
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setSuccess({
        ...
        description: "El ingreso se eliminó.",
      });
    } catch (err) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <FinanceMetricCard
          ...
          detail="Ingresos registrados este año."
        />
        <FinanceMetricCard
          icon={WalletCards}
          label="Ingreso mensual"
          value={formatCurrency(monthly)}
          detail="Entradas registradas este mes."
          tone="blue"
        />
        <FinanceMetricCard
          icon={CalendarClock}
          ...
          value={lastDate}
          detail={`${income.length} movimientos registrados.`}
          tone="amber"
        />
      </div>
          ...
          <CardContent>
            {income.length ? (
              ...
              <p className="py-10 text-center text-sm text-muted-foreground">
                No hay ingresos todavía. Agrega el primero desde el formulario.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        fields={[
          ...
          },
          {
            ...
            options: categories.map((item) => ({
              value: item,
              label: INCOME_CATEGORY_LABELS[item],
            })),
          },
          {
            id: "amount",
